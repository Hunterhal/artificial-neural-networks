...
class cart_pole():
    ...
    def reset(self):
        ...
        self.state[0, :] = np.random.uniform(low=-0.05, high=0.05, size=(4,))
        self.epoch += 1
    ...
    def step(self, action):
        force = np.where(action> 0, self.force, -self.force)
        ...
        polemass_length = self.l * self.mp
        temp = (force + polemass_length * theta_dot ** 2 * sintheta) / m_total
        thetaacc = (self.g * sintheta - costheta * temp) / (self.l * (4.0 / 3.0 - self.mp * costheta ** 2 / m_total))
        xacc = temp - polemass_length * thetaacc * costheta / m_total

        self.iteration_index += 1

        self.state[self.iteration_index, 0] = x + self.beta * x_dot
        self.state[self.iteration_index, 2] = x_dot + self.beta * xacc
        self.state[self.iteration_index, 1] = theta + self.beta * theta_dot
        self.state[self.iteration_index, 3] = theta_dot + self.beta * thetaacc
